...
use super::components::{Ball, Pickup};
    ...
    for collision_event in collisions {
      match collision_event {
        CollisionEvent::Started(collider1, collider2, _) => {
          self.handle_collision_start::<Ball, Pickup>(scene, collider1, collider2);
        }

        CollisionEvent::Stopped(collider1, collider2, _) => {
          self.handle_collision_stop::<Ball, Pickup>(scene, collider1, collider2);
        }
      }
    }
  }
